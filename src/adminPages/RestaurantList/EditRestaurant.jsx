...
import { EditRestaurantSchema } from "@/schema/RestaurantSchema";
import { Button } from "@/components/ui/button";
import { useMutation, useQueryClient } from "@tanstack/react-query";
import { editrestaurant } from "../AdminAPI";
import { toast } from "@/components/ui/use-toast";

export default function EditRestaurant({ restaurant }) {
  const queryClient = useQueryClient();
  const [imagePreview, setImagePreview] = useState(
    import.meta.env.VITE_APP_CLOUDINARY_IMAGE_URL + restaurant.imageId
  );
  const { mutate } = useMutation({
    mutationFn: editrestaurant,
    onSuccess: () => {
      toast({
        description: "Restaurant Updated Sucessfully",
      });
  ...
  const form = useForm({
    ...
    defaultValues: {
      ...
      costForTwo: restaurant.costForTwo.substring(1).split(" ")[0],
    },
  });
  ...
  const onSubmit = async (data) => {
    ...
    mutate({ formData: formData, resId: restaurant._id });
  };
